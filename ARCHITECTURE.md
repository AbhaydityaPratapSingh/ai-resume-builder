...
**Known limitation, undiscovered until the JD test set grew past 30 postings:** the ambiguous "c" skill's own boundary check doesn't treat "+" or "#" as word-boundary-blocking, so "C++" and "C#" both also register a spurious bare "c" match. Harmless-ish for C++ (plausibly implies C fundamentals) but a real precision defect for C# (a different language) — documented and reproduced by a test in `tests/analysis/parseJD.test.js`, not yet fixed. A related bug in the same area *was* fixed while building that test set: a trailing period right after a symbol-ending alias ("...experience with C++.") was being swallowed by the same boundary mechanism and silently dropping the match — see `shared/text/matcher.js`'s `KEEP_LEFT`/`KEEP_RIGHT` split.

**Four more fixes, found once real postings from an actual placement portal (Haveloc) entered the test set** — synthetic JDs, however carefully written, hadn't hit any of these: `minPercentage` had no eligibility-context requirement at all and once grabbed an unrelated "maximum of 30% occupancy of the role" sentence ahead of the real "minimum 73-75% aggregate" cutoff later in the same posting — fixed to require a "minimum" or "aggregate/marks/throughout" context word nearby, not just any bare "N%" anywhere in the document. `gradYear` only matched trigger-word-then-year order ("graduating in 2026") and missed two independent real postings that write the year first ("2027 graduating Batch", "2027 Passout") — fixed to try both orders. The bare "IT" branch check matched the pronoun "it" in ordinary prose (a real posting's "you already use it daily... check it" produced a false `branches: ["cse"]` on a JD with no branch restriction at all) — fixed to require the actual capitalized abbreviation, "IT", not "it"/"It". A real posting named disciplines only to illustrate that it doesn't restrict by branch at all ("we don't shortlist based on branch... whether you studied Computer Science, Electronics, or something entirely different") and still had those names read as an eligibility restriction, the opposite of the JD's own stated intent — fixed with a branch-agnostic-phrase check. That check is proximity-scoped, not whole-document: a branch match is cleared only when *every* occurrence of that branch's name sits within 150 characters of an openness phrase like "any branch" or "we don't shortlist based on branch" (`BRANCH_AGNOSTIC_WINDOW` in `parseJD.js`). A single posting covering multiple roles with different policies ("Sales: open to any branch. SDE: Computer Science, Electronics only.") keeps the real, far-away SDE restriction rather than having an unrelated openness statement elsewhere in the same document wipe it out — a real risk in the first whole-document version of this fix, caught before it shipped rather than after, and covered by a dedicated test. A narrower residual gap remains undocumented no longer: capitalized "IT" used as a department/audit-type name (a real posting titled "Assurance – IT Data & Analytics") still isn't distinguishable from "IT" as an eligible-branch restriction without deeper context modeling than this parser does.
...
- [x] Resume import from PDF (rule-based, with review screen) — entry-level splitting depends on the source PDF's bullets carrying a glyph (•, -, etc.); resumes without one degrade to one entry per line, still reviewable, never dropped. Fixed a real bug found against an actual user PDF: a bullet whose sentence wraps across two lines (no glyph on the continuation, same shape as a new entry's header) was fragmenting a 2-3 project resume into dozens of fake entries — `shared/import/parseResumeText.js`'s `splitEntries`/`splitEducationEntries`/`parseAchievements` now share an `isContinuationLine` heuristic (no sentence-ending punctuation on the line above, or the new line is a stray punctuation fragment) that merges a wrapped line into the entry above it instead of starting a new one. That rule alone was too eager (short unpunctuated lines like "CGPA: 9.78" or a role/date line merged whole sections into one entry), so a wrap now also requires the line above to run near full text width or the new line to start lowercase (`makeWrapTest`). Text extraction (`backend/src/services/pdfImport.js`) now rebuilds spacing from glyph positions, restoring spaces pdf-parse drops at font changes and marking right-aligned columns with a 3-space gap; the parser reads dates/locations from that column and groups LinkedIn-style "company + total duration, then roles" experience layouts. Three anonymized real resumes are hand-labelled in `tests/import/realResumes.test.js`
...
- [x] Analysis test set grown to 44 JDs (`tests/analysis/parseJD.test.js`): 30 written in the style of real Indian campus-placement postings (LinkedIn/Naukri aren't reachable from this repo's tooling), plus 14 real postings — IBM, a fintech Data Science internship, NatWest, Deloitte, Edgro, Bain Capability Network, two ResNet Solutions roles, an EA/Slingshot Studios game-dev internship (one pulled from a .docx), two Codity.ai graduate-trainee roles, HyperVerge's DL/ML research internship, Reltio's program-manager internship, and an unnamed startup's program internship — supplied directly from the user's campus placement portal (Haveloc), across three batches. Each entry, synthetic or real, was independently read and hand-labelled before checking against the parser's actual output, not the other way around. Real postings are a stronger source whenever available — replace synthetic entries with real ones as they come in, rather than just adding more
