...
- [x] Analysis test set grown to 39 JDs (`tests/analysis/parseJD.test.js`): 30 written in the style of real Indian campus-placement postings (LinkedIn/Naukri aren't reachable from this repo's tooling), plus 9 real postings — IBM, a fintech Data Science internship, NatWest, Deloitte, Edgro, Bain Capability Network, two ResNet Solutions roles, and an EA/Slingshot Studios game-dev internship (one pulled from a .docx) — supplied directly from the user's campus placement portal (Haveloc). Each entry, synthetic or real, was independently read and hand-labelled before checking against the parser's actual output, not the other way around. Real postings are a stronger source whenever available — replace synthetic entries with real ones as they come in, rather than just adding more
